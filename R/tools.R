...
tool_get_df_summary <- tool(
  function(object_name) {
    if (!exists(object_name, envir = .GlobalEnv, inherits = FALSE)) {
      return(paste0("Object '", object_name, "' is niet gevonden."))
    }
  ...
  arguments = list(
    object_name = type_string(
      "De naam van het R-object (data frame) dat samengevat moet worden."
    )
  )
)
...
tool_get_colnames <- tool(
  ...
  arguments = list(
    object_name = type_string(
      "De naam van het data frame."
    )
  )
)

if (requireNamespace("plot2", quietly = TRUE)) {
  #' @importFrom ellmer tool type_string type_enum type_number type_integer type_boolean type_ignore
  tool_plot2 <- tool(
    plot2::plot2,
    name = "plot2",
    description = paste0(

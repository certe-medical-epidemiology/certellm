...
test_that("get_chat_object() returns the mock object when an LLM is set", {
  old_obj <- certellm:::pkg_env$chat_object
  mock    <- make_mock_chat()
  certellm:::pkg_env$chat_object <- mock
  on.exit(certellm:::pkg_env$chat_object <- old_obj, add = TRUE)
...
test_that("get_tools() emits 'Tools are disabled' and returns NULL when tools off", {
  old_obj  <- certellm:::pkg_env$chat_object
  old_tool <- certellm:::pkg_env$tools_supported
  certellm:::pkg_env$chat_object    <- make_mock_chat()
  certellm:::pkg_env$tools_supported <- FALSE
  on.exit({
    certellm:::pkg_env$chat_object    <- old_obj
    certellm:::pkg_env$tools_supported <- old_tool
  }, add = TRUE)

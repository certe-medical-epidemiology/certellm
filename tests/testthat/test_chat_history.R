...
test_that("list_chats() returns an empty character vector when history is NULL", {
  old <- certellm:::pkg_env$chat_history
  certellm:::pkg_env$chat_history <- NULL
  on.exit(certellm:::pkg_env$chat_history <- old, add = TRUE)
...
test_that("list_chats() returns all saved snapshot names", {
  old <- certellm:::pkg_env$chat_history
  certellm:::pkg_env$chat_history <- list(snap1 = list(), snap2 = list(), snap3 = list())
  on.exit(certellm:::pkg_env$chat_history <- old, add = TRUE)
...
test_that("list_chats() preserves insertion order of names", {
  old <- certellm:::pkg_env$chat_history
  certellm:::pkg_env$chat_history <- list(z = list(), a = list(), m = list())
  on.exit(certellm:::pkg_env$chat_history <- old, add = TRUE)
...
test_that("save_chat() errors when no LLM is initiated", {
  old_obj <- certellm:::pkg_env$chat_object
  certellm:::pkg_env$chat_object <- NULL
  on.exit(certellm:::pkg_env$chat_object <- old_obj, add = TRUE)
...
test_that("save_chat() stores turns under the supplied name", {
  old_obj  <- certellm:::pkg_env$chat_object
  old_hist <- certellm:::pkg_env$chat_history
  certellm:::pkg_env$chat_object  <- make_mock_chat()
  certellm:::pkg_env$chat_history <- NULL
  on.exit({
    certellm:::pkg_env$chat_object  <- old_obj
    certellm:::pkg_env$chat_history <- old_hist
  }, add = TRUE)

  mock_turns <- list("turn_a", "turn_b")
  certellm:::pkg_env$chat_object$set_turns(mock_turns)

  expect_message(save_chat("my_snap"), "Chat saved as 'my_snap'")
  expect_true("my_snap" %in% names(certellm:::pkg_env$chat_history))
  expect_equal(certellm:::pkg_env$chat_history[["my_snap"]], mock_turns)
})
...
test_that("save_chat() creates the chat_history list when it does not exist yet", {
  ...
  expect_false(is.null(certellm:::pkg_env$chat_history))
  expect_true("first" %in% names(certellm:::pkg_env$chat_history))
})
...
test_that("save_chat() can save multiple snapshots under different names", {
  ...
  expect_equal(sort(names(certellm:::pkg_env$chat_history)), c("snap_a", "snap_b"))
})
...
test_that("save_chat() overwrites an existing snapshot with the same name", {
  ...
  old_hist <- certellm:::pkg_env$chat_history
  certellm:::pkg_env$chat_object  <- make_mock_chat()
  certellm:::pkg_env$chat_history <- list(old_snap = list("old_turn"))
  on.exit({
    certellm:::pkg_env$chat_object  <- old_obj
    certellm:::pkg_env$chat_history <- old_hist
  }, add = TRUE)

  certellm:::pkg_env$chat_object$set_turns(list("new_turn"))
  suppressMessages(save_chat("old_snap"))
  expect_equal(certellm:::pkg_env$chat_history[["old_snap"]], list("new_turn"))
})
...
test_that("save_chat() default name follows YYYYMMDD_HHMMSS format", {
  ...
  nms <- names(certellm:::pkg_env$chat_history)
...
test_that("restore_chat() errors when snapshot name is not in history", {
  ...
  old_hist <- certellm:::pkg_env$chat_history
  certellm:::pkg_env$chat_object  <- make_mock_chat()
  certellm:::pkg_env$chat_history <- list(exists = list())
  on.exit({
    certellm:::pkg_env$chat_object  <- old_obj
    certellm:::pkg_env$chat_history <- old_hist
  }, add = TRUE)
...
test_that("restore_chat() error message mentions list_chats()", {
  ...
  old_hist <- certellm:::pkg_env$chat_history
  certellm:::pkg_env$chat_object  <- make_mock_chat()
  certellm:::pkg_env$chat_history <- list()
  on.exit({
    certellm:::pkg_env$chat_object  <- old_obj
    certellm:::pkg_env$chat_history <- old_hist
  }, add = TRUE)
...
test_that("restore_chat() restores the correct turns", {
  ...
  certellm:::pkg_env$chat_object  <- make_mock_chat()
  certellm:::pkg_env$chat_history <- list(mysnapshot = saved_turns)
  on.exit({
    certellm:::pkg_env$chat_object  <- old_obj
    certellm:::pkg_env$chat_history <- old_hist
  }, add = TRUE)
  ...
  expect_equal(certellm:::pkg_env$chat_object$get_turns(), saved_turns)
})
...
test_that("restore_chat() replaces current turns with the saved ones", {
  ...
  old_hist <- certellm:::pkg_env$chat_history
  certellm:::pkg_env$chat_object  <- make_mock_chat()
  certellm:::pkg_env$chat_history <- list(old = list("old_a", "old_b"))
  certellm:::pkg_env$chat_object$set_turns(list("current"))
  on.exit({
    certellm:::pkg_env$chat_object  <- old_obj
    certellm:::pkg_env$chat_history <- old_hist
  }, add = TRUE)
  ...
  expect_equal(certellm:::pkg_env$chat_object$get_turns(), list("old_a", "old_b"))
})
...
test_that("restore_chat() returns invisible NULL", {
  ...
  old_hist <- certellm:::pkg_env$chat_history
  certellm:::pkg_env$chat_object  <- make_mock_chat()
  certellm:::pkg_env$chat_history <- list(s = list())
  on.exit({
    certellm:::pkg_env$chat_object  <- old_obj
    certellm:::pkg_env$chat_history <- old_hist
  }, add = TRUE)
...
test_that("reset_chat() clears all turns", {
  old_obj <- certellm:::pkg_env$chat_object
  certellm:::pkg_env$chat_object <- make_mock_chat()
  certellm:::pkg_env$chat_object$set_turns(list("turn1", "turn2", "turn3"))
  on.exit(certellm:::pkg_env$chat_object <- old_obj, add = TRUE)

  expect_message(reset_chat(), "Conversation history cleared")
  expect_length(certellm:::pkg_env$chat_object$get_turns(), 0L)
})
...
test_that("reset_chat() succeeds when turns are already empty", {
  ...
  expect_message(reset_chat(), "Conversation history cleared")
  expect_equal(certellm:::pkg_env$chat_object$get_turns(), list())
})
...
test_that("reset_chat() does not touch saved chat_history snapshots", {
  ...
  old_hist <- certellm:::pkg_env$chat_history
  certellm:::pkg_env$chat_object  <- make_mock_chat()
  certellm:::pkg_env$chat_history <- list(kept = list("t"))
  certellm:::pkg_env$chat_object$set_turns(list("active"))
  on.exit({
    certellm:::pkg_env$chat_object  <- old_obj
    certellm:::pkg_env$chat_history <- old_hist
  }, add = TRUE)
  ...
  expect_equal(names(certellm:::pkg_env$chat_history), "kept")
})
...
test_that("save/reset/restore round-trip preserves the conversation", {
  ...
  certellm:::pkg_env$chat_object$set_turns(original_turns)
  ...
  expect_equal(certellm:::pkg_env$chat_object$get_turns(), original_turns)
})
